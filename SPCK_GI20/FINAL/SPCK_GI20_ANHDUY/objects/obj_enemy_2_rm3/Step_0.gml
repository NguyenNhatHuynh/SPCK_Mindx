...
if (distance_to_object(obj_knight) <= 500 && state == 0) {  
    // Cập nhật trạng thái để bắn mũi tên
    state = 1;

    // Tính toán hướng từ obj_enemy1_rm2 đến obj_player
    var _dir = point_direction(x, y, obj_knight.x, obj_knight.y);
    ...
    var _bul = instance_create_layer(x, y, "Instances", obj_attack_enemy_2);
    with (_bul) {
        sprite_index = spr_attack_enemy3_rm3;  // Gán sprite cho mũi tên
